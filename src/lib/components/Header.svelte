<script lang='ts'>
  ...
  import { Home, LayoutDashboard, RotateCcw } from '@lucide/svelte'
...
<header class='sticky top-4 z-50 mb-10 w-full'>
  <div class='relative flex items-center justify-center'>
    <nav class='w-full group relative overflow-hidden rounded-[2rem] border border-white/5 bg-gradient-to-br from-base-200/80 to-base-300/80 p-px shadow-2xl transition-all duration-500 hover:shadow-primary/5'>
      <div class='relative flex items-center justify-between rounded-[31px] bg-base-200/40 px-4 py-3 backdrop-blur-xl sm:px-6'>
        ...
        <div class='flex flex-1 items-center justify-end'>
          {#if page.url.pathname === '/kadro'}
            ...
            >
              <RotateCcw size={14} class='transition-transform group-hover/reset:rotate-[-120deg]' />
              <span>Sıfırla</span>
            </button>
          {:else}
            <a
